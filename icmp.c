...
int icmp_send_host_unreachable(char *data, int len)
{
	...
	if (!DEV_NR(i).enabled)
	    continue;
	DEBUG(LOG_DEBUG, 0,
	      "Sending HOST_UNREACHABLE to %s, len=%d",
	      ip_to_str(DEV_NR(i).ipaddr), len);

	dst_addr.sin_addr = DEV_NR(i).ipaddr;

	ret = sendto(icmp_socket, icmp_send_buf,
		   len + sizeof(struct icmphdr), 0,
		   (struct sockaddr *) &dst_addr, sizeof(dst_addr));
	if (ret < 0)
	    alog(LOG_WARNING, errno, "Could not sent ICMP msg to %s",
