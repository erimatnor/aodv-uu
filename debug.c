...
extern int log_to_file, log_rt_table, rt_log_interval;
...
char *rreq_flags_to_str(u_int8_t flags) {
  static char buf[4];
  int len = 0;
  char *str;
...
char *rrep_flags_to_str(u_int8_t flags) {
  static char buf[3];
  int len = 0;
  char *str;
...
void log_pkt_fields(AODV_msg *msg) {
  ...
  case AODV_RREQ:
    rreq = (RREQ *)msg;
    log(LOG_DEBUG, 0, "rreq->flags:%s rreq->hopcount=%d rreq->flood_id=%ld", 
	rreq_flags_to_str(rreq->flags), rreq->hcnt, rreq->flood_id);
    log(LOG_DEBUG, 0, "rreq->dest_addr:%s rreq->dest_seqno=%lu", 
	ip_to_str(ntohl(rreq->dest_addr)), rreq->dest_seqno);
    log(LOG_DEBUG, 0, "rreq->source_addr:%s rreq->source_seqno=%ld", 
	ip_to_str(ntohl(rreq->src_addr)), rreq->src_seqno);
    break;
  case AODV_RREP:
    rrep = (RREP *)msg;
    log(LOG_DEBUG, 0, "rrep->flags:%s rrep->hcnt=%d", 
	rrep_flags_to_str(rrep->flags), rrep->hcnt); 
    log(LOG_DEBUG, 0, "rrep->dest_addr:%s rrep->dest_seqno=%d", 
	ip_to_str(ntohl(rrep->dest_addr)), rrep->dest_seqno);
    log(LOG_DEBUG, 0, "rrep->src_addr:%s rrep->lifetime=%d", 
	ip_to_str(ntohl(rrep->src_addr)), rrep->lifetime);
    break;
  case AODV_RERR:
    rerr = (RERR *)msg;
...
void print_rt_table() {
  ...
  len += sprintf(rt_buf, "# Time: %02d:%02d:%02d.%03ld\n", time->tm_hour,
		 time->tm_min, time->tm_sec, now.tv_usec / 1000);
  len += sprintf(rt_buf+len, "%-15s %-15s %-3s %-3s %-5s %-8s %-5s %-15s\n", "Destination", "Next hop", "HC", "LHC", "Seqno", "Expire", "Flags", "Precursors");
  ...
  for(i = 0; i < RT_TABLESIZE; i++) {
	...
	len += sprintf(rt_buf+len, "%-15s %-15s %-3d %-3d %-5u %-8llu %-5s\n", 
		       ip_to_str(entry->dest), ip_to_str(entry->next_hop), 
		       entry->hcnt, entry->last_hcnt, entry->dest_seqno, 
		       ((entry->hcnt == 255) ? 0 : entry->expire-get_currtime()), 
		       rt_flags_to_str(entry->flags)); 
      else {
	len += sprintf(rt_buf+len, "%-15s %-15s %-3d %-3d %-5u %-8llu %-5s %-15s\n", 
		       ip_to_str(entry->dest), ip_to_str(entry->next_hop), 
		       entry->hcnt, entry->last_hcnt, entry->dest_seqno, 
		       entry->expire-get_currtime(), 
		       rt_flags_to_str(entry->flags), 
	...
	for(pr = entry->precursors->next; (pr != NULL) ; pr = pr->next)
	  len += sprintf(rt_buf+len, "%60s %-15s\n", "*", 
