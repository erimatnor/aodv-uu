...
void log_pkt_fields(AODV_msg *msg) {
  ...
  case AODV_RREQ:
    rreq = (RREQ *)msg;
    log(LOG_DEBUG, 0, "rreq->flags:%s rreq->hopcount=%d rreq->rreq_id=%ld", 
...
void print_rt_table() {
  ...
  len += sprintf(rt_buf, "# Time: %02d:%02d:%02d.%03ld IP: %s seqno: %u\n", time->tm_hour, time->tm_min, time->tm_sec, now.tv_usec / 1000, ip_to_str(this_host->ipaddr), this_host->seqno);
  ...
  for(i = 0; i < RT_TABLESIZE; i++) {
	...
	len += sprintf(rt_buf+len, "%-15s %-15s %-3d %-3d %-5u %-8llu %-5s\n", 
		       ip_to_str(entry->dest_addr), ip_to_str(entry->next_hop), 		       entry->hcnt, entry->last_hcnt, entry->dest_seqno, 
		       ((entry->hcnt == 255) ? 0 : entry->expire-get_currtime()), rt_flags_to_str(entry->flags)); 
      else {
	len += sprintf(rt_buf+len, "%-15s %-15s %-3d %-3d %-5u %-8llu %-5s %-15s\n", 
		       ip_to_str(entry->dest_addr), ip_to_str(entry->next_hop), 
