...
static ipq_queue_element_t *ipq_dequeue(ipq_queue_t * q,
					int (*cmp) (ipq_queue_element_t *,
						    unsigned long),
					unsigned long data)
{
    struct list_head *i;
...
static ipq_queue_t *ipq_create_queue(nf_queue_outfn_t outfn,
				     ipq_send_cb_t send_cb, int *errp,
				     int *sysctl_qmax)
{
    int status;
    ipq_queue_t *q;
...
static int
ipq_enqueue(ipq_queue_t * q, struct sk_buff *skb, struct nf_info *info)
{
    ipq_queue_element_t *e;
    int status;
...
static int
ipq_set_verdict(ipq_queue_t * q, ipq_verdict_msg_t * v, unsigned int len)
{
    ipq_queue_element_t *e;
...
static int
ipq_receive_peer(ipq_queue_t * q, ipq_peer_msg_t * m,
		 unsigned char type, unsigned int len)
{
	...
	if (m->msg.verdict.value > NF_MAX_VERDICT)
	    ...
	    status = ipq_set_verdict(q, &m->msg.verdict, len - sizeof(*m));
	break;
    default:
	status = -EINVAL;
    }
 ...
 */
static int
netfilter_receive(struct sk_buff *skb, struct nf_info *info, void *data)
{
...
static struct sk_buff *netlink_build_message(ipq_queue_element_t * e,
					     int *errp)
{
	...
	*errp = -EINVAL;
    ...
    pm->hook = e->info->hook;
    if (e->info->indev)
	strcpy(pm->indev_name, e->info->indev->name);
    else
	pm->indev_name[0] = '\0';
    if (e->info->outdev)
	strcpy(pm->outdev_name, e->info->outdev->name);
    else
	pm->outdev_name[0] = '\0';
	...
	if (e->skb->dev->hard_header_parse)
	    pm->hw_addrlen =
		e->skb->dev->hard_header_parse(e->skb, pm->hw_addr);
    }
    if (data_len)
...
static __inline__ void netlink_receive_user_skb(struct sk_buff *skb)
{
    ...
    if (nlh->nlmsg_pid <= 0 || !(nlh->nlmsg_flags & NLM_F_REQUEST)
...
static int
receive_event(struct notifier_block *this, unsigned long event, void *ptr)
{
...
ipq_get_info(char *buffer, char **start, off_t offset, int length)
{
    ...
    len = sprintf(buffer,
		  ...
		  nlq->peer.copy_mode,
		  nlq->peer.copy_range,
		  nlq->len, *nlq->maxlen, nlq->flushing, nlq->terminate);
...
static int __init init(void)
{
    ...
    nfnl =
	netlink_kernel_create(NETLINK_FIREWALL, netlink_receive_user_sk);
    if (nfnl == NULL) {
