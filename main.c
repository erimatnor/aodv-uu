...
void usage(int status)
{
    ...
    printf
	("\nUsage: %s [-dghjlouwxLDRV] [-i if0,if1,..] [-r N] [-n N]\n\n"
	 ...
	 "-R, --rate-limit        Enable rate limiting of RREQs and RERRs.\n"
...
struct sockaddr_in *get_if_info(char *ifname, int type)
{
	...
	log(LOG_ERR, errno, __FUNCTION__,
...
void host_init(char *ifname)
{
	...
	if (ioctl(if_sock, SIOCGIFINDEX, &ifreq) < 0) {
	    log(LOG_ERR, errno, __FUNCTION__, "Could not get index of %s",
		iface);
	    close(if_sock);
	    exit(-1);
	}
    ...
    if (internet_gw_mode) {
	if (find_default_gw()) {
	    log(LOG_NOTICE, 0, __FUNCTION__, "Internet gateway mode enabled!");
...
void signal_handler(int type)
{
    ...
    switch (type) {
    case SIGSEGV:
	log(LOG_ERR, 0, __FUNCTION__, "SEGMENTATION FAULT!!!! Exiting!!! "
...
int main(int argc, char **argv)
{
	...
	opt = getopt_long(argc, argv, "i:jln:dghoq:r:s:uwxDLRV", longopts, 0);
	...
	case 'V':
	    printf
		("\nAODV-UU v%s, AODV draft v%s © Uppsala University & Ericsson AB.\nAuthor: Erik Nordström, <erik.nordstrom@it.uu.se>\n\n",
