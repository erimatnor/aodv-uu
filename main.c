...
void usage(int status)
{
	...
	("\nUsage: %s [-dghjluwxDV] [-i if0,if1,..] [-r N] [-n N]\n\n"
	 ...
	 "-D, --disable-worb      Disable 15 seconds wait on reboot delay.\n"
...
{
    struct sockaddr_in *ina;
    char buf[1024], tmp_ifname[IFNAMSIZ], ifnames[(IFNAMSIZ+1)*MAX_NR_INTERFACES], *iface;
    struct ifconf ifc;
	...
	DEV_IFINDEX(ifreq.ifr_ifindex).broadcast = 
	    ntohl(ina->sin_addr.s_addr);
...
int main(int argc, char **argv)
{
	...
	opt = getopt_long(argc, argv, "i:jln:dgr:uwhxDV", longopts, 0);
	...
	case 'D':
	    ...
	    break;
	case 'V':
	    printf("\nAODV-UU v%s, AODV draft v10 © Uppsala University & Ericsson Telecom AB.\nAuthor: Erik Nordström, erik.nordstrom@it.uu.se\n\n", AODV_UU_VERSION);
	    exit(0);
	    break;
	case '?':
	case ':':
	...
	if ((n = select(nfds, &rfds, NULL, NULL, NULL)) < 0) {
		...
		log(LOG_WARNING, errno,
		    "main.c: Failed select (main loop)");
	    continue;
	}
