...
void aodv_socket_init() {
  ...
  /* Create a UDP socket */
#ifdef RAW_SOCKET
  aodv_socket = socket(PF_INET, SOCK_RAW, IPPROTO_UDP);
#else
  aodv_socket = socket(PF_INET, SOCK_DGRAM, 0);
#endif
  ...
  retval = bind(aodv_socket, (struct sockaddr *)&aodv_addr, 
  ...
  if(setsockopt(aodv_socket, SOL_SOCKET, SO_BROADCAST, &on, sizeof(int)) < 0) {
    perror("aodv_socket_init: Setsockopt failed ");
    exit(-1);
  }
  ...
  if(setsockopt(aodv_socket, SOL_IP, IP_HDRINCL, &on, sizeof(int)) < 0) {
    perror("aodv_socket_init: Setsockopt IP_HDRINCL failed ");
    exit(-1);
  }
#else
  
  if(setsockopt(aodv_socket, SOL_IP, IP_PKTINFO, &on, sizeof(int)) < 0) {
    perror("aodv_socket_init: Setsockopt IP_PKTINFO failed ");
    exit(-1);
  }
  if(setsockopt(aodv_socket, SOL_IP, IP_RECVTTL, &on, sizeof(int)) < 0) {
  ...
  for(;; bufsize -= 1024) {
    if(setsockopt(aodv_socket, SOL_SOCKET, SO_RCVBUF, 
		  (char *)&bufsize, sizeof(bufsize)) == 0) {
      log(LOG_NOTICE, 0, "aodv_socket_init: Receive buffer size set to %d", 
	  bufsize);
      break;
    }
    if(bufsize < RECV_BUF_SIZE) {
      log(LOG_ERR, 0, "aodv_socket_init: Could not set receive buffer size");
      exit(-1);
    }
  }
  
  retval = attach_callback_func(aodv_socket, aodv_socket_read);
...
static void aodv_socket_read(int fd) {
  ...
  len = recvfrom(aodv_socket, recv_buf, RECV_BUF_SIZE, 0, NULL, NULL);
  ...
  udph = (struct udphdr *)(recv_buf + IPHDR_SIZE);
  
  if(ntohs(udph->dest) != AODV_PORT && ntohs(udph->source) != AODV_PORT)
    return;
  
  /* Ignore messages generated locally */ 
  if (memcmp(&src, &this_host->ipaddr, sizeof(u_int32_t)) == 0)
    return;
  
  aodv_msg = (AODV_msg *)(recv_buf + IPHDR_SIZE + sizeof(struct udphdr));
  ...
  if((len = recvmsg(aodv_socket, &msg, MSG_PEEK)) < 0) {
    perror("err: ");
    log(LOG_WARNING, 0, "aodv_socket_read: recvmsg ERROR!");    
    return;
  }
  /* Read the data payload (i.e. AODV msg) */
   len = recvfrom(aodv_socket, recv_buf, RECV_BUF_SIZE, 0, 
...
/*        ip_to_str(src), ip_to_str(dst), ttl); */
  ...
  if(!(aodv_msg->type == AODV_RREP && (ttl == 1)))
    hello_process_non_hello(aodv_msg, src);
...
void aodv_socket_send(AODV_msg *aodv_msg, u_int32_t dst, int len, u_int8_t ttl) {
  ...
  iph->saddr = htonl(this_host->ipaddr);
  iph->daddr = htonl(dst);
  iph->ttl = ttl;
  ...
  if (setsockopt(aodv_socket, SOL_IP, IP_TTL, &ttl, sizeof(ttl)) < 0) {
  ...
  /* If we braodcast this message we update the time of last broadcast
     to prevent unnecessary broadcasts of HELLO msg's */
  if (dst == AODV_BROADCAST)
    this_host->bcast_time = get_currtime();
  
  retval = sendto(aodv_socket, send_buf, len, 0, 
		  (struct sockaddr *)&dst_addr, sizeof(dst_addr));

  if(retval < 0) {
    log(LOG_WARNING, errno, "aodv_socket_send: Failed send %s to %s", 
	ip_to_str(this_host->ipaddr), ip_to_str(dst));
    return;
  }
#ifdef DEBUG
  /* Do not print hello msgs... */
  if(!(aodv_msg->type == AODV_RREP && (dst == AODV_BROADCAST)))
    log(LOG_INFO, 0, "aodv_socket_send: Sent AODV msg to %s (%d bytes)", 
