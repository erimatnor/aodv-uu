...
void rrep_process(RREP *rrep, int rreplen, u_int32_t ip_src, u_int32_t ip_dst, int ip_ttl) {
  ...
  if(memcmp(&rrep_dest, &this_host->ipaddr, sizeof(u_int32_t)) == 0 &&
     memcmp(&rrep_orig, &this_host->ipaddr, sizeof(u_int32_t)) == 0)
    return;
  ...
  /* Check if this RREP was for us (i.e. we previously made a RREQ
     for this host). */
    if(memcmp(&rrep_orig, &this_host->ipaddr, sizeof(u_int32_t)) == 0) {
  
      /* Remove destination from seeking list since a route has been
         found. */
      seek_list_remove(rrep_dest);
