...
#ifdef DEBUG
	    log(LOG_DEBUG, 0, "link_qual_init: %s added to spy list.", 
...
int ip2mac(u_int32_t ip_addr, struct sockaddr *mac) 
{
    int i;
    u_int32_t n_addr;
    
    n_addr = htonl(ip_addr);

    for (i = 0; i < nipmac; i++) {
	if (memcmp(&n_addr, &ip2mac_table[i].ip, sizeof(u_int32_t)) == 0) {
...
static int link_qual_get(struct sockaddr *mac_addr, char *ifname)
{
  ...
  /*   char mac1[128], mac2[128]; */
	...
	fprintf(stderr, "link_qual_get: Could not get link quality : %s\n", strerror(errno));
	exit(-1);
    }
...
int link_qual_add_spy(struct sockaddr *mac_addr, char *ifname)
{
	...
	fprintf(stderr, "link_qual_set_spy: Bad MAC address!\n");
	return(-1);
    }
	...
	fprintf(stderr, "link_qual_set_spy: SIOCSIWSPY failed!\n");
	return(-1);
    }
    return 0;
}
...
int link_qual_add_spy_by_ip(struct sockaddr *ip_addr, char *ifname) 
{
    ...
    if ((ioctl(iwsock, SIOCGARP, &arpq) < 0) ||
       !(arpq.arp_flags & ATF_COM)) {
	  fprintf(stderr, "Arp failed for %s, %s : %s\nTry to ping the address before setting it.\n", inet_ntoa(((struct sockaddr_in *)ip_addr)->sin_addr), ifname, strerror(errno));
	  return(-1);
    }
    
      /* Store address pairs */
    if (nipmac > IW_MAX_SPY) {
	fprintf(stderr, "link_qual_set_spy_by_ip: IW_MAX_SPY reached!!\n");
	return(-1);
    } 
  
    memcpy(&ip2mac_table[nipmac].ip, &sin->sin_addr.s_addr, sizeof(u_int32_t));
    memcpy(ip2mac_table[nipmac].mac, &arpq.arp_ha.sa_data, ETH_ALEN);
    nipmac++;
    
    if (link_qual_add_spy(&arpq.arp_ha, ifname) < 0) {
	fprintf(stderr, "link_qual_set_spy_by_ip: Set spy failed...\n");
	return(-1);
    }
    return 0;
}
