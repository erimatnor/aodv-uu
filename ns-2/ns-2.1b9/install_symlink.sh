...
# "Installs" patched ns files into ns source code tree (like old install.sh),
# but create symlinks to the modified files instead of overwriting old files
# with copies of the modified files.
...
for file in `find ./ -type f -print | grep -v "[CVS|~]" | grep -v .sh$`; do
    file=${file#./}
    echo "$ns_path/$file -symlink-> $currdir/$file"
