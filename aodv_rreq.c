...
void NS_CLASS rreq_send(u_int32_t dest_addr, u_int32_t dest_seqno, int ttl,
			u_int8_t flags)
{
    RREQ *rreq;
    int i;
...
void NS_CLASS rreq_forward(RREQ * rreq, int ttl)
{
    int i;
    /* FORWARD the RREQ if the TTL allows it. */
    DEBUG(LOG_INFO, 0, "forwarding RREQ src=%s, rreq_id=%lu",
	  ip_to_str(ntohl(rreq->orig_addr)), ntohl(rreq->rreq_id));

    /* Queue the received message in the send buffer */
    rreq = (RREQ *) aodv_socket_queue_msg((AODV_msg *) rreq, RREQ_SIZE);
			   ...
			   u_int32_t ip_dst, int ip_ttl, unsigned int ifindex)
{
	...
	case RREQ_EXT:
	    ...
	    break;
	default:
	    log(LOG_WARNING, 0, __FUNCTION__, "Unknown extension type %d",
		ext->type);
	    break;
	}
	...
	else if (rev_rt->next_hop != ip_src) {
	...
	if (fwd_rt && fwd_rt->state == VALID && !rreq->d &&
	    !(fwd_rt->flags & RT_INV_SEQNO) &&
	    (rreq->u || fwd_rt->dest_seqno >= rreq_dest_seqno)) {
	    ...
	    if (fwd_rt && !(fwd_rt->flags & RT_INV_SEQNO) &&
		(rreq->u || fwd_rt->dest_seqno > rreq_dest_seqno)) {
...
void NS_CLASS rreq_local_repair(rt_table_t * rt, u_int32_t src_addr,
				struct ip_data *ipd)
{
    struct timeval now;
    seek_list_t *seek_entry;
							  ...
							  u_int32_t rreq_id)
{
    ...
    timer_set_timeout(&rreq_pkt->rec_timer, PATH_DISCOVERY_TIME);
    return rreq_pkt;
}
...
struct blacklist *NS_CLASS rreq_blacklist_insert(u_int32_t dest_addr)
{
    ...
    timer_set_timeout(&bl_entry->bl_timer, BLACKLIST_TIMEOUT);
    return bl_entry;
}
