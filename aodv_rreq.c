...
static struct rreq_record *rreq_record_insert(u_int32_t orig_addr, u_int32_t rreq_id);
static struct rreq_record *rreq_record_find(u_int32_t orig_addr, u_int32_t rreq_id);

static struct blacklist *rreq_blacklist_head = NULL;
struct blacklist *rreq_blacklist_find(u_int32_t dest_addr);

extern int unidir_hack, rreq_gratuitous, use_expanding_ring_search;
...
RREQ *rreq_create(u_int8_t flags, u_int32_t dest_addr, 
  ...
  rreq->orig_addr = htonl(this_host->ipaddr);
  ...
  log(LOG_DEBUG, 0, "rreq_create: Assembled RREQ %s", ip_to_str(dest_addr));
...
void rreq_process(RREQ *rreq, int rreqlen, u_int32_t ip_src, u_int32_t ip_dst, int ip_ttl) {
  ...
  if(internet_gw_mode && 
     ...
     (rreq_dest & this_host->netmask) != 
     (this_host->ipaddr & this_host->netmask)) {
  ...
  /* Are we the destination of the RREQ?, if so we should immediately send a
     RREP.. */
  if (memcmp(&rreq_dest, &this_host->ipaddr, sizeof(u_int32_t)) == 0) {
  ...
  if(fwd_rt == NULL) {
    if(ip_ttl > 1)
      goto forward_rreq;
  } else if(fwd_rt->dest_seqno < rreq_dest_seqno)
    goto forward_rreq;
  else
    goto generate_rrep;
 ...
 forward_rreq:
  ...
  /* FORWARD the RREQ (AODV draft v.10, section 6.5). */
  
  if(fwd_rt != NULL && (rreq_dest_seqno < fwd_rt->dest_seqno))
    rreq_dest_seqno = fwd_rt->dest_seqno;
  
  
  log(LOG_INFO, 0, "rreq_process: forwarding RREQ src=%s, rreq_id=%lu", 
 ...
 generate_rrep:    
  ...
  rrep = rrep_create(flags, 0, fwd_rt->hcnt, rreq_dest, fwd_rt->dest_seqno,
		     rreq_orig, lifetime);
  ...
  if(rreq->g) {
    rrep = rrep_create(flags, 0, rev_rt->hcnt, rreq_orig, rreq_orig_seqno, 
		       rreq_dest, fwd_rt->expire - get_currtime());
    aodv_socket_send((AODV_msg *)rrep, rreq_dest, RREP_SIZE, MAXTTL);
...
void rreq_route_discovery(u_int32_t dest_addr, u_int8_t flags, 
  ...
  if(use_expanding_ring_search)
    seek_entry->timer_id = timer_new(2*ttl*NODE_TRAVERSAL_TIME,
  ...
  rreq = rreq_create(flags, dest_addr, dest_seqno);
...
void rreq_local_repair(rt_table_t *rt_entry) {
  ...
  /* Remember that we are seeking this destination */
  seek_entry = seek_list_insert(rt_entry->dest_addr, rt_entry->dest_seqno, 
				ttl, flags, NULL);
...
#endif
  rreq_pkt->timer_id = timer_new(PATH_TRAVERSAL_TIME, rreq_record_timeout, 
				 rreq_pkt);
  return rreq_pkt;
}
