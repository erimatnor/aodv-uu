							  ...
							  u_int32_t rreq_id)
{
    ...
    rec->rreq_id = rreq_id;
    rec->rec_timer.handler = &NS_CLASS rreq_record_timeout;
    rec->rec_timer.data = rec;
							...
							u_int32_t rreq_id)
{
	...
	if (rec->orig_addr == orig_addr && (rec->rreq_id == rreq_id))
	    return rec;
    }
    return NULL;
}
...
struct blacklist *NS_CLASS rreq_blacklist_insert(u_int32_t dest_addr)
{
    ...
    bl->bl_timer.handler = &NS_CLASS rreq_blacklist_timeout;
    bl->bl_timer.data = bl;
