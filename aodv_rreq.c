...
RREQ *NS_CLASS rreq_create(u_int8_t flags, u_int32_t dest_addr,
			   u_int32_t dest_seqno, u_int32_t orig_addr)
{
	...
	rreq->g = 1;
    DEBUG(LOG_DEBUG, 0, "rreq_create: Assembled RREQ %s", ip_to_str(dest_addr));
#ifdef DEBUG_OUTPUT
...
void NS_CLASS rreq_process(RREQ * rreq, int rreqlen, u_int32_t ip_src,
			   u_int32_t ip_dst, int ip_ttl, unsigned int ifindex)
{
    ...
    DEBUG(LOG_DEBUG, 0,
	  "RREQ_process: ip_src=%s rreq_orig=%s rreq_dest=%s",
	...
	switch (ext->type) {
	case RREQ_EXT:
	    DEBUG(LOG_INFO, 0, "rreq_process: RREQ include EXTENSION");
	    /* Do something here */
	    break;
	default:
    ...
    /* Calculate the extended minimal life time as defined in section
       6.5, draft 10. */
    life = PATH_TRAVERSAL_TIME - 2 * (rreq->hcnt + 1) * NODE_TRAVERSAL_TIME;


    if (rev_rt == NULL) {
	DEBUG(LOG_DEBUG, 0, "rreq_process: rev_rt = NULL; route not found");

	rev_rt = rt_table_insert(rreq_orig, ip_src, rreq->hcnt + 1,
	...
	if (((rreq_orig_seqno > rev_rt->dest_seqno) ||
	    ...
	    /* If the expire time of the route is larger than the minimal
	       ...
	       i.e. life = 0 */
	    if (rev_rt->hcnt != INFTY &&
		timeval_diff(&rev_rt->rt_timer.timeout, &minimal_lifetime) > 0)
		life = 0;

	    rev_rt = rt_table_update(rev_rt, ip_src, rreq->hcnt + 1,
  ...
  /**** END updating/creating REVERSE route ****/
    ...
    /* Are we the destination of the RREQ?, if so we should immediately send a
       RREP.. */
    if (memcmp(&rreq_dest, &DEV_IFINDEX(ifindex).ipaddr,
	       sizeof(u_int32_t)) == 0) {
	...
	if ((rev_rt->hcnt != INFTY && rev_rt->flags & UNIDIR) ||
	    ((rreq->hcnt + 1) == 1 && unidir_hack)) {
		...
		timer_timeout_now(&neighbor->rt_timer);
	...
	rrep = rrep_create(flags, 0, fwd_rt->hcnt, rreq_dest,
	...
	if (rreq->g) {
	    rrep = rrep_create(flags, 0, rev_rt->hcnt, rreq_orig,
	...
	DEBUG(LOG_INFO, 0, "rreq_process: forwarding RREQ src=%s, rreq_id=%lu",
...
void NS_CLASS rreq_route_discovery(u_int32_t dest_addr, u_int8_t flags,
				   struct ip_data *ipd)
{
    RREQ *rreq;
    struct timeval now;
	...
	/* DEBUG(LOG_INFO, 0, "rreq_route_discovery: Already seeking %s", */
/* 	      ip_to_str(dest_addr)); */
    ...
    if (rt_entry == NULL)
	dest_seqno = 0;
    else if (rt_entry->hcnt == INFTY || (rt_entry->flags & UNIDIR)) {
	...
	if (expanding_ring_search && rt_entry->last_hcnt != INFTY)
	...
	if (timeval_diff(&rt_entry->rt_timer.timeout, &now) / 1000 <
	    PATH_TRAVERSAL_TIME)
	    rt_table_update_timeout(rt_entry, PATH_TRAVERSAL_TIME);

    } else {
	DEBUG(LOG_INFO, 0,
	      "rreq_route_discovery: Route already exist for %s??",
	      ip_to_str(dest_addr));
    ...
    if (expanding_ring_search)
	timer_add_msec(&seek_entry->seek_timer, 2 * ttl * NODE_TRAVERSAL_TIME);
    else
	timer_add_msec(&seek_entry->seek_timer, NET_TRAVERSAL_TIME);

    DEBUG(LOG_DEBUG, 0,
	  "rreq_route_discovery: Seeking %s ttl=%d", ip_to_str(dest_addr), ttl);
...
void NS_CLASS rreq_local_repair(rt_table_t * rt_entry)
{
    RREQ *rreq;
    seek_list_t *seek_entry;
    int ttl, i;
    ...
    rt_entry->flags |= LREPAIR;

    ttl = (int) (max(rt_entry->last_hcnt, 0.5 * 1) + LOCAL_ADD_TTL);
    ...
    DEBUG(LOG_DEBUG, 0,
	  "rreq_local_repair: Seeking %s ttl=%d",
	  ip_to_str(rt_entry->dest_addr), ttl);
...
NS_STATIC struct rreq_record *NS_CLASS rreq_record_insert(u_int32_t orig_addr,
							  u_int32_t rreq_id)
{
    ...
    if ((rreq_pkt =
	 (struct rreq_record *) malloc(sizeof(struct rreq_record))) < 0) {
	fprintf(stderr, "rreq_record_insert: Malloc failed!!!\n");
	exit(-1);
    }
    rreq_pkt->orig_addr = orig_addr;
    rreq_pkt->rreq_id = rreq_id;
    rreq_pkt->rec_timer.handler = &NS_CLASS rreq_record_timeout;
...
NS_STATIC struct rreq_record *NS_CLASS rreq_record_find(u_int32_t orig_addr,
							u_int32_t rreq_id)
{
    struct rreq_record *rreq_pkt;
    ...
    while (rreq_pkt != NULL) {
	if (rreq_pkt->orig_addr == orig_addr && (rreq_pkt->rreq_id == rreq_id))
	    return rreq_pkt;
...
int NS_CLASS rreq_record_remove(u_int32_t orig_addr, u_int32_t rreq_id)
{
...
struct blacklist *NS_CLASS rreq_blacklist_insert(u_int32_t dest_addr)
{
    ...
    if ((bl_entry = (struct blacklist *) malloc(sizeof(struct blacklist))) < 0) {
	fprintf(stderr, "rreq_blacklist_insert: Malloc failed!!!\n");
	exit(-1);
    }
    bl_entry->dest_addr = dest_addr;
    bl_entry->bl_timer.handler = &NS_CLASS rreq_blacklist_timeout;
...
struct blacklist *NS_CLASS rreq_blacklist_find(u_int32_t dest_addr)
{
    struct blacklist *bl_entry;
...
int NS_CLASS rreq_blacklist_remove(u_int32_t dest_addr)
{
