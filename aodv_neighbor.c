                           ...
                           unsigned int ifindex)
{
    ...
    if (!rt) {
        ...
        rt = rt_table_insert(source, source, 1, 0, ACTIVE_ROUTE_TIMEOUT, VALID,
                             0, ifindex);
    } else {
        ...
        rt_table_update(rt, source, 1, seqno, ACTIVE_ROUTE_TIMEOUT, VALID,
                        rt->flags);
    }
...
void NS_CLASS neighbor_link_break(rt_table_t *rt)
{
    ...
    if (rerr) {
        ...
        if (rt_u && rerr->dest_count == 1 && rerr_unicast_dest.s_addr)
            aodv_socket_send((AODV_msg *)rerr, rerr_unicast_dest,
            ...
            for (i = 0; i < MAX_NR_INTERFACES; i++) {
                ...
                aodv_socket_send((AODV_msg *)rerr, dest, RERR_CALC_SIZE(rerr),
                                 1, &DEV_NR(i));
            }
        }
    }
