...
typedef struct rt_table {
  ...
  u_int8_t iface;         /* An index to a network interface... */
  ...
  u_int64_t expire;       /* Last time in millisecs (absolute time)
			     since the link to the dest was used */
  u_int16_t last_life;    /* The last lifetime... */
  u_int16_t flags;        /* Routing flags */
  u_int32_t timer_id;       /* Id of the timer associated with this entry */
  u_int32_t ack_timer_id; /* RREP_ack timer for this destination */
  u_int32_t hello_timer_id;
  u_int64_t last_hello_time;
  u_int8_t hello_cnt;
  hash_value hash;
...
rt_table_t* rt_table_update(rt_table_t *rt_entry, u_int32_t next, 
			    u_int8_t hops, u_int32_t seqno, u_int32_t newlife, 
			    u_int16_t flags);
