...
extern int llfeedback, active_route_timeout, qual_threshold, internet_gw_mode, wait_on_reboot;
...
/* 		  ip_to_str(dest_addr)); */
	...
	case KAODVM_SEND_RERR:
		...
		DEBUG(LOG_DEBUG, 0, "Sending RERR for unsolicited message from %s to dest %s", ip_to_str(src_addr), ip_to_str(dest_addr));
		...
		aodv_socket_send((AODV_msg *) rerr, rerr_dest,
				 RERR_CALC_SIZE(rerr), 1, &DEV_IFINDEX(mu->ifindex));
