 ...
 */
static inline struct kaodv_queue_entry *__kaodv_queue_find_entry(kaodv_queue_cmpfn cmpfn, unsigned long data)
{
	struct list_head *p;
...
static inline struct kaodv_queue_entry *__kaodv_queue_find_dequeue_entry(kaodv_queue_cmpfn cmpfn, unsigned long data)
{
...
static struct kaodv_queue_entry *kaodv_queue_find_dequeue_entry(kaodv_queue_cmpfn cmpfn, unsigned long data)
{
	struct kaodv_queue_entry *entry;
...
int kaodv_queue_enqueue_packet(struct sk_buff *skb, int (*okfn)(struct sk_buff *))
{
	...
	if (entry == NULL) {
		printk(KERN_ERR "ip_queue: OOM in kaodv_queue_enqueue_packet()\n");
		return -ENOMEM;
	}
