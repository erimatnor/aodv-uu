				 ...
				 struct iphdr *iph)
{
	...
	res = ifi_from_ifindex(NULL, &bcaddr, dev->ifindex);
			       ...
			       int (*okfn) (struct sk_buff *))
{
	...
	if (hooknum == NF_IP_PRE_ROUTING)
		res = ifi_from_ifindex(&ifaddr, &bcaddr, in->ifindex);
	else 
		res = ifi_from_ifindex(&ifaddr, &bcaddr, out->ifindex);
	...
	case NF_IP_LOCAL_OUT:
		...
		if (!kaodv_expl_get(iph->daddr, &e) ||
			...
			/* Make sure the maximum segment size (MSM) is
			   ...
			   nicest way to do it. It works for sometime,
			   but may freeze due to some locking issue I
			   guess. Need to fix... */
