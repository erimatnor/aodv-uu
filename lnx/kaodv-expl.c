...
int
kaodv_expl_add(__u32 daddr, __u32 nhop, unsigned long time,
...
static int kaodv_expl_print(char *buf)
{
	...
	len +=
	    sprintf(buf, "# %-15s %-15s %-5s Expires\n", "Addr", "Nhop",
		    "Flags");
	...
	list_for_each(pos, &expl_head) {
		...
		len +=
		    sprintf(buf + len, "  %-15s %-15s %-5s %lu\n", addr, nhop,
			    flags, (e->expires - jiffies) * 1000 / HZ);
	}
...
void
kaodv_expl_update(__u32 daddr, __u32 nhop, unsigned long time,
		  unsigned short flags)
{
	struct expl_entry *e;
