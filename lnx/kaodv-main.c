...
static void kaodv_update_route_timeouts(int hooknum, const struct net_device *dev, struct iphdr *iph)
{
    struct expl_entry e;
    ...
    else if (iph->daddr != INADDR_BROADCAST &&
	     iph->daddr != netdevs[dev->ifindex].bc_addr)
			       ...
			       int (*okfn) (struct sk_buff *))
{
    ...
    for (i = 0; i < nif; i++)
	    if (iph->saddr == netdevs[i].bc_addr)
		    return NF_ACCEPT;
	...
	for (i = 0; i < nif; i++)
		...
		if (is_gateway && iph->protocol == IPPROTO_MIPE &&
		    ...
		    (*skb) = ip_pkt_decapsulate(*skb);
		    iph = (*skb)->nh.iph;
		}
		/* Ignore packets generated locally or that are for this
		 * node. */
		else if (iph->saddr == netdevs[i].ip_addr ||
			 iph->daddr == netdevs[i].ip_addr)
			return NF_ACCEPT;
	...
	for (i = 0; i < nif; i++)
		...
		if (!kaodv_expl_get(iph->daddr, &e) ||
			...
			kaodv_netlink_send_rt_msg(KAODVM_NOROUTE, iph->saddr, iph->daddr);
