...
kaodv-arm: 
	$(MAKE) -C $(AODVDIR)/lnx kaodv-arm.o KERNEL_DIR=$(KERNEL_DIR) KCC=$(ARM_CC) LD=$(ARM_LD)

kaodv-mips: 
	$(MAKE) -C $(AODVDIR)/lnx kaodv-mips.o KERNEL_DIR=$(KERNEL_DIR) KCC=$(MIPS_CC) LD=$(MIPS_LD)
...
docs:
	cd docs && $(MAKE) all
clean: 
	rm -f aodvd *~ *.o core *.log $(NS_TARGET) endian endian.h ns/*.o ns/*~
	cd lnx && $(MAKE) clean
#cd docs && $(MAKE) clean
