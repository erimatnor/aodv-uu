			    ...
			    u_int16_t flags, unsigned int ifindex)
{
    ...
    if (hops != INFTY) {
	ret = k_add_rte(dest_addr, next, 0, hops, rt_entry->ifindex);
...
int rt_table_invalidate(rt_table_t * rt_entry)
{
  int ret;
	...
	log(LOG_DEBUG, 0, "route_table_invalidate: last HELLO: %ld", 
    ...
    if (rt_entry->hcnt == INFTY) {
#ifdef DEBUG
	log(LOG_DEBUG, 0, "route_table_invalidate: Route %s already invalidated!!!", ip_to_str(rt_entry->dest_addr));
#endif
	return -1;
    }
