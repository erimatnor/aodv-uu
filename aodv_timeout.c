...
#define TTL_VALUE seek_entry->ttl
    ...
    if (seek_entry->reqs < RREQ_RETRIES) {
        ...
        DEBUG(LOG_DEBUG, 0, "Seeking %s ttl=%d wait=%d",
              ip_to_str(seek_entry->dest_addr), TTL_VALUE,
              2 * TTL_VALUE * NODE_TRAVERSAL_TIME);
        ...
        rreq_send(seek_entry->dest_addr, seek_entry->dest_seqno, TTL_VALUE,
                  seek_entry->flags);
...
void NS_CLASS route_expire_timeout(void *arg)
{
    ...
    DEBUG(LOG_DEBUG, 0, "Route %s DOWN, seqno=%d", ip_to_str(rt->dest_addr),
          rt->dest_seqno);
