...
void NS_CLASS hello_send(void *arg)
{
	...
	for (i = 0; i < MAX_NR_INTERFACES; i++) {
...
#endif
			    memcpy(AODV_EXT_NEXT(ext), &rt->dest_addr, sizeof(struct in_addr));
	    ...
	    aodv_socket_send((AODV_msg *) rrep, dest, msg_size,
			     1, &DEV_NR(i));
	}
...
void NS_CLASS hello_process(RREP * hello, int rreplen, unsigned int ifindex)
{
	...
	case RREP_HELLO_NEIGHBOR_SET_EXT:
		...
		ext_neighbor.s_addr = *(in_addr_t *)((char *) AODV_EXT_DATA(ext) + i);
    ...
    if ((rt = rt_table_find(hello_dest)) == NULL) {
	...
	rt_table_update(rt, hello_dest, 1, hello_seqno, timeout, VALID, 
			flags | rt->flags);
    }
