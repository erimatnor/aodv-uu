...
#define ROUTE_TIMEOUT_SLACK 200
...
void hello_process(RREP * hello, int rreplen, unsigned int ifindex)
{
	...
	if (unidir_hack && unidir_link) {
	    rt_entry =
		rt_table_insert(hello_dst, hello_dst, 1, hello_seqno,
				ALLOWED_HELLO_LOSS * HELLO_INTERVAL + 
				ROUTE_TIMEOUT_SLACK,
				NEIGHBOR | UNIDIR, ifindex);
#ifdef DEBUG
	...
	if (rt_entry->flags & UNIDIR) {
	    ...
	    rt_table_update_timeout(rt_entry,
				    ALLOWED_HELLO_LOSS * HELLO_INTERVAL + 
				    ROUTE_TIMEOUT_SLACK);
	} else
...
#define HELLO_DELAY 100		/* The extra time we should allow an hello
