...
{
    DEBUG(LOG_DEBUG, 0, "No active forwarding routes - stopped sending HELLOs!");
...
void NS_CLASS hello_send(void *arg)
{
	...
	for (i = 0; i < MAX_NR_INTERFACES; i++) {
		...
		continue;
#ifdef DEBUG_HELLO
	    DEBUG(LOG_DEBUG, 0, "sending Hello to %s",
...
void NS_CLASS hello_process(RREP * hello, int rreplen, unsigned int ifindex)
{
    ...
    u_int32_t ext_neighbor, timeout;
	...
	case RREP_HELLO_NEIGHBOR_SET_EXT:
		...
		if (ext_neighbor == DEV_IFINDEX(ifindex).ipaddr)
		    flags &= ~RT_UNIDIR;
	    }
	    break;
	default:
    ...
    if ((rt = rt_table_find(hello_dst)) == NULL) {
	/* No active or expired route in the routing table. So we add a
	   new entry... */
	
	rt = rt_table_insert(hello_dst, hello_dst, 1, hello_seqno, timeout,
				 state, flags, ifindex);
	...
	rt_table_update(rt, hello_dst, 1, hello_seqno, timeout, VALID, 
			flags);
    }
...
NS_INLINE void NS_CLASS hello_update_timeout(rt_table_t * rt, long time)
{
